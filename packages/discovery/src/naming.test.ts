...
describe('analyzeEndpoint', () => {
  ...
  it('reads an item', () => {
    expect(anatomyOf(['/api/todos/a1', '/api/todos/b2'])).toEqual({
      shape: 'item',
      entity: 'todo',
    });
  });
...
describe('nameOperation', () => {
  const cases: [string, Parameters<typeof nameOperation>[0], string, string][] = [
    [
      'create',
      { method: 'POST', anatomy: anatomyOf(['/api/todos']), queryKeys: [], hasRequestBody: true },
      'createTodo',
      'create',
    ],
    [
      'list',
      { method: 'GET', anatomy: anatomyOf(['/api/todos']), queryKeys: [], hasRequestBody: false },
      'listTodos',
      'list',
    ],
    [
    ...
    [
      ...
      {
        method: 'GET',
        anatomy: anatomyOf(['/api/todos/a1', '/api/todos/b2']),
        queryKeys: [],
        hasRequestBody: false,
      },
      'getTodo',
    ...
    [
      ...
      {
        method: 'POST',
        anatomy: anatomyOf(['/api/todos/a1/archive', '/api/todos/b2/archive']),
        queryKeys: [],
        hasRequestBody: false,
      },
      'archiveTodo',
  ...
  it('pluralises irregular entities correctly', () => {
    expect(
      nameOperation({
        method: 'GET',
        anatomy: anatomyOf(['/api/categories']),
        queryKeys: [],
        hasRequestBody: false,
      }).name,
    ).toBe('listCategories');
  ...
  it('recognises a session endpoint as sign in and sign out', () => {
    ...
    expect(
      nameOperation({ method: 'POST', anatomy, queryKeys: [], hasRequestBody: true }).name,
    ).toBe('signIn');
    expect(
      nameOperation({ method: 'DELETE', anatomy, queryKeys: [], hasRequestBody: false }).name,
    ).toBe('signOut');
  });
